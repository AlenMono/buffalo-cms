...
const TestimonialBlock = ({ testimonialText, testimonialAuthor, testimonialPlace }: TestimonialBlockProps) => {
    return (
        <div className='flex flex-col items-center gap-10 lg:gap-20 text-center'>
            <QuoteSvg aria-hidden="true" className="w-9 h-9 md:w-16 md:h-16" />

            <p className='section-title font-faustina-italic'>{testimonialText}</p>

            <div>
                <p className='font-faustina-italic text-lg md:text-2xl font-semibold'>{testimonialAuthor}</p>
                <p className='text-sm md:text-base text-brand-mid'>{testimonialPlace}</p>
            </div>
        </div>
    )
}
...
const QuoteSvg = ({ className, ...props }: { className?: string } & React.SVGProps<SVGSVGElement>) => {
    return <svg width="64" height="64" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg" className={className} {...props}>
